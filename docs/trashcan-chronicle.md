...
- Learned the atomic-update keep-list only protects /etc — a kernel cannot
...
| Display on D300s | ✅ via amdgpu.dc=0 (legacy DCE6 path) |
| Vulkan | ✅ 1.3 on both GPUs via RADV |
| Desktop session | ✅ full Plasma **Wayland** — kwin_wayland runs fine on GFX6 |
...
**Final state:** power button → GRUB (5 s menu) → SI kernel → both D300s up →
